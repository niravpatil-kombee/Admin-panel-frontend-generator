...
const sanitizeFieldName = (name: string): string => name.replace(/[^a-zA-Z0-9_]/g, '_');
function mapToTsType(zodType: Field['zodType']): string {
    switch (zodType) { case 'number': return 'number'; case 'boolean': return 'boolean'; case 'date': return 'string'; case 'any': return 'any'; default: return 'string'; }
}
...
export function generateListPage(modelName: string, fields: Field[]): void {
  ...
  const primaryDisplayField = fieldsForListing.find(f => f.fieldName.toLowerCase() === 'name') || fieldsForListing[0] || fields[0];
  ...
  const modelTypeDefinition = `\nexport type ${modelTypeName} = {\n  ${fields.map(field => `${sanitizeFieldName(field.fieldName)}: ${mapToTsType(field.zodType)};`).join('\n  ')}\n};`;
  const mockData = `\nconst mockData: ${modelTypeName}[] = [\n  ${Array.from({ length: 13 }, (_, i) => `{ ${fields.map(f => { const sName = sanitizeFieldName(f.fieldName); if (sName === 'id') return `id: ${i + 1}`; return `${sName}: ${mapToTsType(f.zodType) === 'number' ? 10 + i : `"${capitalize(f.fieldName.replace(/_/g, ' '))} ${i + 1}"`}`; }).join(', ')} }`).join(',\n  ')}\n];`;
...
import { ArrowUpDown, Plus, Eye, Pencil, Trash2, ChevronLeft, ChevronRight, Upload, FileDown } from "lucide-react"
...
export function ${componentName}() {
  ...
  const columns: ColumnDef<${modelTypeName}>[] = React.useMemo(() => [
    ...
    ${fieldsForListing.map(field => `{ 
      accessorKey: "${sanitizeFieldName(field.fieldName)}", 
      header: ({ column }) => (${field.sortable ? `
        <Button variant="ghost" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>
          ${field.label} <ArrowUpDown className="ml-2 h-4 w-4" />
        </Button>` : `<span>${field.label}</span>`})
    }`).join(',\n    ')},
    { id: "actions", header: () => <div className="text-right">Actions</div>, cell: ({ row }) => (<div className="flex items-center justify-end gap-1"><Button variant="ghost" size="icon" title="View" onClick={() => setViewingRow(row.original)}><Eye className="h-4 w-4" /></Button><Button asChild variant="ghost" size="icon" title="Edit"><Link to={\`/${singleModel}/edit/\${row.original.id}\`}><Pencil className="h-4 w-4" /></Link></Button><Button variant="ghost" size="icon" title="Delete" className="text-red-600" onClick={() => handleDeleteRow(row.original.id)}><Trash2 className="h-4 w-4" /></Button></div>) },
  ], []);
      ...
      <Dialog open={!!viewingRow} onOpenChange={isOpen => !isOpen && setViewingRow(null)}>
        <DialogContent className="sm:max-w-md p-0">
          ...
          <div className="border-y"><div className="grid auto-rows-min gap-y-4 p-6">
            ${fieldsForDialog.map(field => {
              const sanitizedName = sanitizeFieldName(field.fieldName);
              return `<div className="grid grid-cols-2 items-start gap-x-4"><span className="text-muted-foreground">${field.label}</span><p className="font-medium">{String(viewingRow?.${sanitizedName} ?? '')}</p></div>`;
            }).join('')}
          </div></div>
          <DialogFooter className="sm:justify-end gap-2 p-6 pt-4">
            <Button variant="outline" onClick={() => setViewingRow(null)}>Cancel</Button>
            <Button asChild><Link to={\`/${singleModel}/edit/\${viewingRow?.id}\`}><Pencil className="mr-2 h-4 w-4" /> Edit</Link></Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
